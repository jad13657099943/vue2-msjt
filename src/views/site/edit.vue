...
import { RegionUpdate} from "../../require/api";
export default {
  components: {
    back,
    vantSwitch: Switch,
  ...
  methods: {
    ...
    add() {
      ...
      Dialog.confirm({
        ...
        RegionUpdate(this.params).then((res) => {
          Toast(res.msg);
          setTimeout(() => {
            this.$router.go(-1);
          }, 1000);
  ...
  created() {
        ...
        this.params.region=item.region;
        this.params.content=item.content;
        this.params.is_default=item.is_default;
        this.params.mobile=item.mobile;
        this.params.name=item.name;
        this.site=item.province+item.city+item.region;
        this.params.id=item.id;
        if(item.is_default>1) this.check=true;
  },
};
</script>
